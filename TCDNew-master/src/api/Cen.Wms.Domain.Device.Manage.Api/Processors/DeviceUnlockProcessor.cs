...
{
    public class DeviceUnlockProcessor: IQueryProcessor<DeviceIdentifierDto, RpcResponse<bool>>
    {
        private readonly ILogger _logger;
        private readonly UnitOfWork<WmsContext> _unitOfWork;
        private readonly IDeviceRepository _deviceRepository;

        public DeviceUnlockProcessor(
        ...
        public async Task<RpcResponse<bool>> Run(IUserIdProvider userIdProvider, DeviceIdentifierDto request)
        {
            ...
            if (deviceStateResult.Data != DeviceStatus.Locked)
                return RpcResponse<bool>.WithError(false, CommonErrors.InvalidOperation);
            
            var deviceStateSetResult = await _deviceRepository.DeviceStatusSet(request, DeviceStatus.Active);
