...
{
    public class RegistrationRequestAcceptProcessor: IQueryProcessor<ByIdReq, RpcResponse<bool>>
    {
        ...
        public RegistrationRequestAcceptProcessor(
        ...
        public async Task<RpcResponse<bool>> Run(IUserIdProvider userIdProvider, ByIdReq request)
        {
            ...
            var acceptRequestResult = await _registrationRequestRepository.RegistrationRequestAccept(request);
            if (!acceptRequestResult.IsSuccess)
                return acceptRequestResult;
            
            var setDeviceStateResult = await _deviceRepository.DeviceStatusSet(deviceIdentifier, DeviceStatus.Active);
