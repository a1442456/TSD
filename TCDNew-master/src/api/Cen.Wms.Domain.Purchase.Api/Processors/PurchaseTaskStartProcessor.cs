...
{
    public class PurchaseTaskStartProcessor : IQueryProcessor<ByIdReq, RpcResponse<bool>>
    {
        private readonly ILogger _logger;
        private readonly UnitOfWork<WmsContext> _unitOfWork;
        private readonly IPurchaseTaskRepository _purchaseTaskRepository;

        public PurchaseTaskStartProcessor(ILogger logger, UnitOfWork<WmsContext> unitOfWork, IPurchaseTaskRepository purchaseTaskRepository)
        {
            _logger = logger;
        ...
        {
            var result = await _purchaseTaskRepository.PurchaseTaskStart(request.Id);
