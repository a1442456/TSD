...
{
    public class PurchaseTaskUserIncludeProcessor : IQueryProcessor<PurchaseTaskUserEditReq, RpcResponse<bool>>
    {
        ...
        public PurchaseTaskUserIncludeProcessor(ILogger logger, UnitOfWork<WmsContext> unitOfWork, 
        ...
        public async Task<RpcResponse<bool>> Run(IUserIdProvider userIdProvider, PurchaseTaskUserEditReq request)
        {
            ...
            var purchaseTaskPacIncludeResult = await _purchaseTaskRepository.PurchaseTaskUserInclude(request.PurchaseTaskId, request.UserId);

            await _unitOfWork.Context.SaveChangesAsync();
            _unitOfWork.Commit();
            
            return purchaseTaskPacIncludeResult;
        }
    }
}