...
{
    public class PurchaseTaskUploadProcessor : IQueryProcessor<ByIdReq, RpcResponse<bool>>
    {
        ...
        public PurchaseTaskUploadProcessor(ILogger logger, UnitOfWork<WmsContext> unitOfWork, IPurchaseTaskRepository purchaseTaskRepository, IPacUploader pacUploader)
        {
            _logger = logger;
        ...
        {
            var result = await _purchaseTaskRepository.PurchaseTaskUpload(request.Id, _pacUploader);
