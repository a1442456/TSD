...
{
    public class PurchaseTaskCancelProcessor : IQueryProcessor<ByIdReq, RpcResponse<bool>>
    {
        private readonly ILogger _logger;
        private readonly UnitOfWork<WmsContext> _unitOfWork;
        private readonly IPurchaseTaskRepository _purchaseTaskRepository;

        public PurchaseTaskCancelProcessor(ILogger logger, UnitOfWork<WmsContext> unitOfWork, IPurchaseTaskRepository purchaseTaskRepository)
        {
            _logger = logger;
        ...
        {
            var result = await _purchaseTaskRepository.PurchaseTaskCancel(request.Id);
