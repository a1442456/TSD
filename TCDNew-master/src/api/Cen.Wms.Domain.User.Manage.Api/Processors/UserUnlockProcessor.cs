...
{
    public class UserUnlockProcessor: IQueryProcessor<ByIdReq, RpcResponse<bool>>
    {
        private readonly ILogger _logger;
        private readonly UnitOfWork<WmsContext> _unitOfWork;
        private readonly IUserRepository _userRepository;

        public UserUnlockProcessor(ILogger logger, UnitOfWork<WmsContext> unitOfWork, IUserRepository userRepository)
        {
            _logger = logger;
        ...
        public async Task<RpcResponse<bool>> Run(IUserIdProvider userIdProvider, ByIdReq request)
        {
            ...
            var userStateSetResult = await _userRepository.UserStateSet(request, new UserStateEditModel { IsLocked = false });
