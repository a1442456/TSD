...
namespace Cen.Common.Http.Server.OpenApi
{
    ...
    {
        private const string GenericSuffixStart = "0FFFFF";
        private const string GenericSuffixDelimiter = "000FFF";
        private const string GenericSuffixEnd = "00FFFF";
        private const string ArraySuffix = "0000FF";
        
        public static RequestDelegate BuildOpenApiResponse(OpenApiOptions options, Dictionary<(string verb, string path), RouteMetaData> metaDatas, OpenApiSpecVersion specVersion)
        {
            return context =>
            {
                ...
                AddSchema(schemaNavigation, document, context, options);
                ...
                document.SerializeAsJson(context.Response.Body, specVersion);
                return Task.CompletedTask;
            };
        }
        ...
        private static void FlatternGenericNames(Dictionary<string, SchemaElement> navigation)
        {
            foreach (var keyValuePair in navigation)
            {
                ...
                keyValuePair.Value.ShortName += FlatternGenericNamePostfix(keyValuePair.Value);
            }
        }

        private static string FlatternGenericNamePostfix(SchemaElement schemaElement)
        {
            var genericSuffix = string.Empty;
            foreach (var genericTypeParam in schemaElement.GenericTypes)
            {
                if (genericSuffix.Length == 0)
                {
                    genericSuffix = GenericSuffixStart;
                }
                else
                {
                    genericSuffix = GenericSuffixDelimiter;
                }

                var typeParamName = genericTypeParam.ShortName + FlatternGenericNamePostfix(genericTypeParam);
                genericSuffix += typeParamName.Replace("[]", ArraySuffix);
                genericSuffix += GenericSuffixEnd;
            }
        ...
        private static OpenApiDocument CreateDocument(OpenApiOptions options)
        {
            return new OpenApiDocument
            {
                ...
                Servers = new List<OpenApiServer>(options.ServerUrls.Select(x => new OpenApiServer { Url = x })),
        ...
        private static OpenApiSchema SchemaFromElement(SchemaElement schemaElement)
        {
            ...
            if (schemaElement.IsSimple())
            {
                ...
                schema.Format = GetOpenApiFormatMapping(schemaElement.ElementType.Name);
            }
            else if (schemaElement.IsSimpleNullable())
            {
                ...
                schema.Format = GetOpenApiFormatMapping(Nullable.GetUnderlyingType(schemaElement.ElementType).Name);
        ...
        private static void AddSecurityInformation(OpenApiOptions options, OpenApiDocument document)
        {
            foreach (var globalSecurity in options.GlobalSecurityDefinitions)
            {
            ...
            foreach (var apiSecurity in options.Securities)
            {
            ...
            foreach (var queryStringParameter in queryStringParameters)
            {
                operation.Parameters.Add(new OpenApiParameter
                {
                    ...
                    Schema = new OpenApiSchema
                    {
                        ...
                        Format = GetOpenApiFormatMapping(queryStringParameter.Type.Name)
                    }
                });
            }
        }
            ...
            if (template.Parameters.Any())
            {
                operation.Parameters = template.Parameters.Select(x => new OpenApiParameter
                {
                    ...
                    Schema = new OpenApiSchema
                    {
                        ...
                        Format = GetOpenApiFormatMapping(x.InlineConstraints.FirstOrDefault()?.Constraint)
                    }
                }).ToList();
            }
        }
        ...
        private static OpenApiResponse CreateOpenApiResponse(Dictionary<string, SchemaElement> navigation, OpenApiDocument document, RouteMetaDataResponse valueStatusCode)
        {
        ...
        private static string GetOpenApiFormatMapping(string constraint)
        {
            if (constraint == null)
            {
                return null;
